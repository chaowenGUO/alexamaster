...
async def f():
    ...
    await asyncio.create_subprocess_exec('apt', 'install', '-y', '--no-install-recommends', './google-chrome-stable_current_amd64.deb', stdout=asyncio.subprocess.PIPE)
    ...
    #sys.exit()
    process =  await asyncio.create_subprocess_exec('google-chrome', '--version')
